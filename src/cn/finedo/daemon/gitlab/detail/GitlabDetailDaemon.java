...
public class GitlabDetailDaemon {
	private static Logger logger = LogManager.getLogger();
	private ApplicationContext springcontext;

	public GitlabDetailDaemon() {
		springcontext = new ClassPathXmlApplicationContext("spring-context.xml");
	}

	@SuppressWarnings("unused")
	public static void main(String[] args) {
		GitlabDetailDaemon statDeamon = new GitlabDetailDaemon();
		try {
			statDeamon.gitlab();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void gitlab() throws Exception {
		...
		GitlabDetailService gitlabDetailService = springcontext
				.getBean(GitlabDetailService.class);
		...
		InputStream in = GitlabDetailDaemon.class.getClassLoader().getResourceAsStream("gitlab2.properties");
		prop.load(in);
		...
		for(BaselineExtends be : bes){
			try {
				...
				if(be.getPath().indexOf(list.get(0).getStr1())>-1||be.getPath().indexOf(list.get(0).getStr2())>-1){
					...
					logger.debug(list.get(0).getUrl() + list.get(0).getUsername()+ list.get(0).getPassword());
					...
					gitlabDetailService.addLogs(logs);
					gitlabDetailService.addLogDetails(logDetails);
				}
				if(be.getPath().indexOf(list.get(0).getStr3())>-1){
					logger.debug(list.get(1).getUrl() + list.get(1).getUsername()+ list.get(1).getPassword());
